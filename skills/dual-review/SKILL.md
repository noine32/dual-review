...
### 自動起動 (層2: 明示語 + 対象指定)
層1 の明示語に**対象指定が伴う場合**のみ自動起動。曖昧語のみの場合は層3 (提案のみ) に降ろす。
- 「Codex にも `src/foo.ts` をレビューさせて」→ `review`（target あり）
- 「Codex にこの計画を批判してもらって」→ `plan` または `critique`（直近文脈で判定）
- 「セカンドオピニオンが欲しい (`gh pr diff` で得られる差分について)」→ `critique`

### 提案のみ (層3: 自動発火しない・Claude が一行で勧める)
以下の語/場面では Claude が**起動を提案する**にとどめ、ユーザー承認を待つ:
- 曖昧語のみ: 「見落としないか」「本当にこれでいい？」「批判的に見て」「もっと良い方法ない？」「他の意見」
  - これらは一般会話でも頻出するため、誤発火回避のため自動起動はしない
- 大きな PR を作る前 / マージ前
