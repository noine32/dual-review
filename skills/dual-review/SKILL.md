...
**Codex の作業ディレクトリ (`CODEX_CD`) の重要な注意**:

Codex は `--sandbox read-only` で起動するため、`--cd` で渡したディレクトリ**配下のファイルしか読めません**。
スクリプトは既定で `$PWD` を渡しますが、**レビュー対象が現在のディレクトリ外にある場合は `CODEX_CD` を設定する**必要があります（さもないと指摘 0 件で返ります）。

```bash
# ✅ 正しい例: 別プロジェクトをレビュー
CODEX_CD=/path/to/target-project /dual review src/foo.ts

# ❌ 間違い: 現在の dir がレビュー対象を含んでいない
cd /home/me/dual-review
/dual review /home/me/another-project/src/foo.ts  # 0 件で返る
```
