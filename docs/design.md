...
- **Status**: Implemented (post-dogfood iterations applied 2026-05-02)
...
2. **並列で** Codex を `gpt-5.2 reasoning=medium` で起動し、同じ対象をレビューさせる
...
**層1（自動起動・明示語のみ）**: 「Codex にも〜」「Codex にレビュー/批判/意見/反論」「Codex と一緒に」「もう一人の AI」「別モデルで」「二者で」「両方の視点」「対立する観点」「赤チーム」「devil's advocate」「セカンドオピニオン」

**層2（自動起動・明示語 + 対象指定）**: 層1 のフレーズに**対象指定が伴う場合**のみ自動起動:
- 「Codex にも `src/foo.ts` をレビューさせて」→ `review`
- 「Codex にこの計画を批判してもらって」→ `plan` または `critique`（直近文脈で判定）

**層3（提案のみ・自動発火しない）**: 以下のような曖昧語/場面では Claude が**起動を一行で提案**するにとどめ、ユーザー承認を待つ:
- 曖昧語: 「見落としないか」「本当にこれでいい？」「批判的に見て」「もっと良い方法」「他の意見」（一般会話で頻出するため誤発火回避）
- 大きな PR 前 / マージ前 / セキュリティ・認証・課金等の sensitive 領域 / アーキテクチャ選定
...
オプション:
- `--reasoning <high|medium|low>`: モデル reasoning 上書き（Plus 制約により `xhigh` は使用不可、実装で allow-list 強制）
- `--model <gpt-5.2>`: モデル上書き（ChatGPT Plus は `gpt-5.2` のみサポート）
...
**ChatGPT Plus 制約**: `gpt-5.2-mini` / `gpt-5.2-max` および `xhigh` reasoning は Plus アカウントで API 拒否される（Pro/Business 限定）ため**未対応**。Plus は `gpt-5.2` のみ。モード差は `reasoning_effort` で表現する。
**共通フラグ**: `--skip-git-repo-check`、stderr は一時ファイル退避＋失敗時のみ末尾表示（過去の `2>/dev/null` 設計は v1 dogfood で問題化したため改訂）
...
実装は dogfood 経由で複数回改訂。**実コードは `/home/noine/dual-review/install.sh` を参照**。要点:

- `pwd -P` で symlink 経由起動時の物理パス正規化
- `CLAUDE_HOME`/`HOME` の両方が未設定なら明示的にエラー (`set -u` の cryptic メッセージを回避)
- パスが `-` で始まる場合を拒否（`ln -s --` を使えない BSD 互換の代替策）
- バックアップ名は `.bak.$(date +%s).$$.${RANDOM}` で衝突回避
- 既存 symlink は `readlink` で参照先を表示してから削除（事故防止）
- `rm`/`mv` は `--` 付き、`ln -s` は `--` なし（BSD ln 互換性）
...
#   review          → gpt-5.2, reasoning=medium  (review モードの並列レビュー側)
...
- env override 反映
- `codex exec --skip-git-repo-check --sandbox read-only -m <model> --config model_reasoning_effort=<level> < <prompt-file> 2>"$STDERR_FILE"`（stderr は mktemp に退避、失敗時に `tail -50` で表示）
...
実 SKILL.md は `/home/noine/dual-review/skills/dual-review/SKILL.md`。frontmatter の方針 (要点):

- **自動起動するのは明示語のみ**（「Codex にも〜」「セカンドオピニオン」「devil's advocate」「赤チーム」等）
- 曖昧語（「見落としないか」「本当にこれでいい？」「批判的に見て」）と sensitive 領域は**提案のみ**で自動発火しない（誤発火防止）
- `description` は層1 の明示語のみ列挙、本文の層3 で曖昧語と提案条件を詳述
...
| レート制限ヒット | エラー解釈、`DUAL_REASONING=low` でリトライ提案を `AskUserQuestion` で確認 |
...
- [ ] ChatGPT Plus 制約内で動く（`gpt-5.2-mini` / `gpt-5.2-max` / `xhigh` を使わない）
- [ ] `bats tests/` が green
